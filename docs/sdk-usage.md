...
svc, err := cliproxy.NewBuilder().
    WithConfig(cfg).
    WithConfigPath("config.toml"). // absolute or working-dir relative
    Build()
if err != nil { panic(err) }
...
- Management endpoints are mounted only when `remote-management.secret-key` is set in `config.toml`.
...
- Hot reload: changes to `config.toml` and `auths/` are picked up automatically.
